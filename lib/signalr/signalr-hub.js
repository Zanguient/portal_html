...
.factory('Hub', ['$', function ($) {
	...
	return function (hubName, options) {
		...
		Hub.connect = function () {
            var op = {};
            if(options.transport) op.transport = options.transport;
            if(options.jsonp) op.jsonp = options.jsonp;
            //console.log(op);
            if(!op.transport && !op.jsonp) op = null;
			return Hub.connection.start(op);//options.transport ? { transport: options.transport } : null);
		};
